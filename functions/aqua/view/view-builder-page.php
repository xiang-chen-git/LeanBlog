...
if ( ! current_user_can('edit_theme_options') )
	wp_die( __( 'Cheatin&#8217; uh?', 'themnific' ) );
...
switch($action) {
	...
	case 'create' :
		...
		if(!is_wp_error($new_id)) {
			...
			$messages[] = '<div id="message" class="updated"><p>' . __('The ', 'themnific') . '<strong>' . $template_name . '</strong>' . __(' page template has been successfully created', 'themnific') . '</p></div>';
		} else {
			$errors = '<ul>';
			...
			$messages[] = '<div id="message" class="error"><p>' . __('Sorry, the operation was unsuccessful for the following reason(s): ', 'themnific') . '</p>' . $errors . '</div>';
		}
	...
	case 'update' :
		...
		$messages[] = '<div id="message" class="updated"><p>' . __('The ', 'themnific') . '<strong>' . $template_name . '</strong>' . __(' page template has been updated', 'themnific') . '</p></div>';
		break;
	...
	case 'delete' :
		...
		$messages[] = '<div id="message" class="updated"><p>' . __('The template has been successfully deleted', 'themnific') . '</p></div>';
		break;
}
	...
	<div id="page-builder-frame">
		...
		<div id="page-builder-column" class="metabox-holder <?php echo $disabled ?>">
			<div id="page-builder-archive" class="postbox">
				<h3 class="hndle"><span><?php _e('Available Blocks', 'themnific') ?></span><span id="removing-block"><?php _e('Deleting', 'themnific') ?></span></h3>
				<div class="inside">
					<ul id="blocks-archive" class="cf">
						<?php $this->blocks_archive() ?>
					</ul>
					<p><?php _e('Need help? Use the Help tab in the upper right of your screen.', 'themnific') ?></p>
				</div>
			</div>
		</div>
		...
		<div id="page-builder-fixed">
					...
					<form id="update-page-template" action="<?php echo $this->args['page_url'] ?>" method="post" enctype="multipart/form-data">
								...
								<div id="submitpost" class="submitbox">
									<div class="major-publishing-actions cf">
										...
										<label class="open-label" for="template-name">
											<span><?php _e('Template Name', 'themnific') ?></span>
										...
										<div class="publishing-action">
											<?php submit_button( empty( $selected_template_id ) ? __( 'Create Template', 'themnific' ) : __( 'Save Template', 'themnific' ), 'button-primary ', 'save_template', false, array( 'id' => 'save_template_header' ) ); ?>
										...
										<div class="delete-action">
											...
											echo '<a class="submitdelete deletion template-delete" href="' . esc_url(add_query_arg(
												...
												admin_url( 'themes.php' )
											)) . '">'. __('Delete Template', 'themnific') .'</a>';
											?>
						...
						<div id="aqpb-body">
							...
							<ul class="blocks cf" id="blocks-to-edit">
								...
								if($selected_template_id === 0) {
									...
									<br/><br/>
									When you have finished building your custom page template, make sure you click the Save Template button.', 'themnific');
									echo '</p>';
						...
						<div id="aqpb-footer">
							<div class="major-publishing-actions cf">
								<div class="publishing-action">
									<?php if(!empty($selected_template_id)) {
										submit_button( __( 'Save Template', 'themnific'), 'button-primary ', 'save_template', false, array( 'id' => 'save_template_footer' ) ); 
									} ?>
								</div><!-- END .publishing-action -->
							</div><!-- END .major-publishing-actions -->
						</div>
			...
			<p style="float:left"><small></small></p>
			<p style="float:right"><small>Aqua Page Builder &copy; 2012 by <a href="http://aquagraphite.com">Syamil MJ</a></small></p>
