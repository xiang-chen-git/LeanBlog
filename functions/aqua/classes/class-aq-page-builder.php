...
if(!class_exists('AQ_Page_Builder')) {
	class AQ_Page_Builder {
		...
		function __construct( $config = array()) {
			...
			$defaults['menu_title'] = __('Page Builder', 'themnific');
			$defaults['page_title'] = __('Page Builder', 'themnific');
			$defaults['page_slug'] = __('aq-page-builder', 'themnific');
		...
		function admin_enqueue() {
			...
			// Register 'em
			wp_register_style( 'aqpb-css',  get_template_directory_uri().'/functions/aqua/assets/css/aqpb.css');
			wp_register_style( 'aqpb-blocks-css', get_template_directory_uri().'/functions/aqua/assets/css/aqpb_blocks.css');
			wp_register_script('aqpb-js', get_template_directory_uri().'/functions/aqua/assets/js/aqpb.js');
			wp_register_script('aqpb-fields-js', get_template_directory_uri().'/functions/aqua/assets/js/aqpb-fields.js');
		...
		function view_enqueue() {
			...
			// front-end css
			wp_register_style( 'aqpb-view-css', get_template_directory_uri().'/functions/aqua/assets/css/aqpb-view.css');
		...
		function register_template_post_type() {
			...
			if(!post_type_exists('template')) {
				...
				add_action('admin_notices', create_function('', "echo '<div id=\"message\" class=\"error\"><p><strong>Aqua Page Builder notice: </strong>'. __('The \"template\" post type already exists, possibly added by the theme or other plugins. Please consult with theme author to consult with this issue', 'themnific') .'</p></div>';"));
			}
		...
		function display_blocks( $template_id ) {
			...
			if(empty($blocks)) {
				...
				echo __('Drag block items from the left into this area to begin building your template.', 'themnific');
				echo '</p>';
				return;
		...
		**/
		function display_template($template_id) {
			...
			if(empty($blocks)) {
				...
				echo '<p class="empty-template">';
				echo __('This template is empty', 'themnific');
				echo '</p>';
		...
		function add_shortcode() {
			...
			if ( !array_key_exists( 'template', $shortcode_tags ) ) {
				...
				add_action('admin_notices', create_function('', "echo '<div id=\"message\" class=\"error\"><p><strong>Aqua Page Builder notice: </strong>'. __('The \"[template]\" shortcode already exists, possibly added by the theme or other plugins. Please consult with the theme author to consult with this issue', 'themnific') .'</p></div>';"));
			}
		...
		function add_media_button( $button ) {
			...
			if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) ) ) {
				...
				if ( version_compare( $wp_version, '3.5', '<' ) ) {
					$img 	= '<img src="' . AQPB_DIR . '/assets/images/aqua-media-button.png" width="16px" height="16px" alt="' . esc_attr__( 'Add Page Template', 'themnific' )  . '" />';
					$output = '<a href="#TB_inline?width=640&inlineId=aqpb-iframe-container" class="thickbox" title="' . esc_attr__( 'Add Page Template', 'themnific' )  . '">' . $img . '</a>';
				} else {
					...
					$output = '<a href="#TB_inline?width=640&inlineId=aqpb-iframe-container" class="thickbox button" title="' . esc_attr__( 'Add Page Template', 'themnific' ) . '" style="padding-left: .4em;">' . $img . ' ' . esc_attr__( 'Add Template', 'themnific' ) . '</a>';
				}
			...
			if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) ) ) {
				...
				<script type="text/javascript">
					function insertTemplate() {
						...
						if ( '' == id ) {
							alert("<?php echo esc_js( __( 'Please select your template first!', 'themnific' ) ); ?>");
							return;
						}
				...
				<div id="aqpb-iframe-container" style="display: none;">
					<div class="wrap" style="padding: 1em">
						...
						if ( empty( $templates ) ) {
							echo sprintf( __( 'You don\'t have any template yet. Let\'s %s create %s one!', 'themnific' ), '<a href="' .admin_url().'themes.php?page=aq-page-builder">', '</a>' );
							return;
						}
						?>						

						<h3><?php _e( 'Choose Your Page Template', 'themnific' ); ?></h3><br />
						...
						<input type="button" id="aqpb-insert-template" class="button-primary" value="<?php echo esc_attr__( 'Insert Template', 'themnific' ); ?>" onclick="insertTemplate();" />
						<a id="aqpb-cancel-template" class="button-secondary" onclick="tb_remove();" title="<?php echo esc_attr__( 'Cancel', 'themnific' ); ?>"><?php echo esc_attr__( 'Cancel', 'themnific' ); ?></a>
		...
		function contextual_help() {
			...
			if($screen->id == $this->page) {
				...
				$screen->set_help_sidebar(
					'<p><strong>' . __('For more information:', 'themnific') . '</strong></p>' .
